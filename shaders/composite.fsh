...
const vec3 skylightColor = vec3(0.05, 0.08, 0.12);
const vec3 sunlightColor = vec3(1.2, 1.1, 1.0);
const vec3 ambientColor = vec3(0.02, 0.02, 0.04);
...
const vec3 NIGHT_FOG = vec3(0.05, 0.06, 0.08);
...
vec3 dynamicFogColor = mix(NIGHT_FOG, SUNSET_FOG, smoothstep(-0.1, 0.1, sunElevation));
dynamicFogColor = mix(dynamicFogColor, DAY_FOG, smoothstep(0.1, 0.3, sunElevation));
if(depth >= 0.9999){
...
vec3 sunVisual = vec3(1.0, 0.9, 0.7) * (sunDisc*5.0 + sunGlow*1.5);
vec3 currentSky = DEAD_SKY_COLOR + sunVisual;
float density = 0.0;
...
sceneColor = mix(currentSky, CLOUD_DARKNESS, density * CLOUD_INTENSITY);
float skyFogFactor = smoothstep(0.0, 0.3, abs(worldDir.y));
sceneColor = mix(dynamicFogColor, sceneColor, skyFogFactor);
float fogScatter = pow(sunDot, 8.0)*(1.0 - skyFogFactor);
sceneColor += vec3(1.0, 0.9, 0.7)*fogScatter*3.0;
}
else{
...
vec3 sunlight = sunlightColor*clamp(dot(sunDir, normal), 0.0, 1.0)*shadow;
...
float terrainFogScatter = pow(terrainSunDot, 6.0)*(1.0-fogFactor);
vec3 finalFogColor = dynamicFogColor + vec3(1.0, 0.9, 0.7)*terrainFogScatter*4.0;
